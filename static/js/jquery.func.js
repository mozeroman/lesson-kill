...
$(function() {
	$(document).on('focusin', '.field, textarea', function() {
		...
		return false;
	}).on('click','#indexposbt_prelong',function(){
		$('html,body').animate({
			scrollTop:0
		},{
			duration:500,
		...
		return false;
	}).on('click','#indexposbt_stats',function(){
		$('html,body').animate({
			scrollTop:300
		},{
			duration:500,
		...
		return false;
	}).on('click','#indexposbt_course',function(){
		$('html,body').animate({
			scrollTop:600
		},{
			duration:500,
		...
		return false;
	}).on('click','#indexposbt_question',function(){
		$('html,body').animate({
			scrollTop:600
		},{
			duration:500,
		...
		return false;
	}).on('click','#indexposbt_teacher',function(){
		$('html,body').animate({
			scrollTop:1100
		},{
			duration:500,
		...
		return false;
	}).on('click','#indexposbt_links',function(){
		$('html,body').animate({
			scrollTop:1500
		},{
			duration:500,
	...
	$(window).load(function() {
		if($('#slider').length) {
			$('#slider .image-slider').flexslider({
				...
				start: function(slider) {
					$('#slider .elements').delay(0).fadeIn(500, function() {
						var $this = $(this);
						// $this.find('.white-box').delay(100).fadeIn(600, function() {
							...
							// $('.flex-direction-nav').delay(200).fadeIn(500);
		...
		if($('.tweets-widget').length) {
			$('.tweets-widget').append('<span class="prev" /><span class="next" />');

			$(".tweets-widget .container > ul").carouFredSel({
				direction: "up",
				height: "variable",
				items: {
					visible: 7,
				...
				auto: false,
				prev: ".tweets-widget .prev",
				next: ".tweets-widget .next"
			});
		}
